...
export default function BuyerPersonasCreate() {
      ...
      try {
        ...
        const content_prompt = `El contenido debe estar en perfecto ${formData.language} y debe estar escrito en HTML. También resalta las frases importantes en negritas.`;
          ...
          const buyerPersonaData = {
              ...
              minWordsInContent: formData.minWordsInContent,
              buyerpersona_prompt,
              title_prompt,
              content_prompt
          };
  
          // Crear un documento de Buyer Persona en Firestore
          const buyerPersonasCollection = collection(getFirestore(), 'buyerpersonas');
          await setDoc(doc(buyerPersonasCollection), buyerPersonaData);
  
          console.log('Buyer Persona creado exitosamente.');
  ...
  return (
    ...
    <Grid item xs={12}>
    <Accordion expanded={isFormVisible} onChange={() => setIsFormVisible(!isFormVisible)}>
  <AccordionSummary
    ...
    <Typography>Crear Buyer Persona</Typography>
      ...
      <Paper sx={{ p: 2, display: 'flex', flexDirection: 'column' }}>        
        <TextField
          ...
          />
          <TextField
            label="Mínimo de palabras en el contenido"
            name="minWordsInContent"
            value={formData.minWordsInContent}
            onChange={handleInputChange}
          />
